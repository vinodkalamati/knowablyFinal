...
public class AnalyticsServiceImpl implements AnalyticsService{
    ...
    public void saveQuery(UserResponse userResponse) {
        ...
        key.setDomain(userResponse.getDomain());
        key.setQuery(userResponse.getQuery());
        if(userResponse.getResponse().equals("accurate"))
        {
            key.setPosResponse(1);
        }
        else if(userResponse.getResponse().equals("inaccurate"))
        {
            key.setNegResponse(1);
        }
        QueryResultResponse resultResponse = new QueryResultResponse();
        resultResponse.setKey(key);
        resultResponse.setResult(userResponse.getResult());
    ...
    public void updateQuery(UserResponse userResponse) {
        QueryResultResponse resultResponse = queryResultResponseRepository.findByKeyQuery(userResponse.getQuery());
    ...
    public void deleteResponse(QueryResultResponse response) {
        queryResultResponseRepository.delete(response);
    }
}
