...
public class AnalyticsServiceController {
    ...
    public ResponseEntity<String> processResponse(@RequestBody UserResponse userResponse){
        List<String> list = new ArrayList<>();
        try{
            //Checks if response already exists in cassandra and updates, else it adds to cassandra
            if(analyticsService.existsByQuery(userResponse.getQuery()))
            {
                analyticsService.updateQuery(userResponse);
            ...
            {
                analyticsService.saveQuery(userResponse);
                //Sends response as json as frontend is expecting json string
    ...
    public ResponseEntity<String> deleteResponse(@RequestBody QueryResultResponse resultResponse) {
        try {
            analyticsService.deleteResponse(resultResponse);
