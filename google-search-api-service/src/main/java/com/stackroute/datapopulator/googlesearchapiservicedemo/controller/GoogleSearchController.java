...
public class GoogleSearchController {
    ...
    public ResponseEntity<String> domainUpload(@RequestBody UiInput input1)
    {
        try {
            ...
            input.setSessionId("not required");
            if(googleSearchService.checkCache(conceptArray))
            {
                ...
                {
                    allFutures.add(googleSearchService.getLinks(input.getSessionId(),null,input1.getUserId(),input1.getDomain(),
                            conceptArray[i].trim()));
                }
    ...
    public ResponseEntity<?> domainUploadInternal(@RequestBody UiInputInt input1)
    {
        try {
            ...
            for(int i=0;i<conceptArray.length;i++)
            {
                allFutures.add(googleSearchService.getLinks(input.getSessionId(),input1.getQuery(),input1.getUserId(), input1.getDomain(),
                        conceptArray[i].trim()));
            }
            CompletableFuture.allOf(allFutures.toArray(new CompletableFuture[0])).join();
            List<SearchResult> list = new ArrayList<>();
