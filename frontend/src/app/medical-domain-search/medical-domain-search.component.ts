...
export class MedicalDomainSearchComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    stompClient.connect({},frame =>{
      stompClient.subscribe('/topic/notification/'+sessionId,notifications=>{
        ...
        if(this.notifications.result != null){
  ...
  userSearch(searchQuery){
    localStorage.setItem('searchQuery', searchQuery.searchTerm);
  ...
  sendMessageFromSpeechRecognition(): void {
    ...
    localStorage.setItem('searchQuery', this.formValue);
    const speech = new Speech()
    speech.speak({
    ...
    this.medicalSearchService.VoiceSearchService(this.formValue)
      .subscribe(data=>{
        console.log(data);
      },error=>{
        console.log(error);
